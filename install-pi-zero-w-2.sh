...
# so the service user reaches the SPI/I2C/GPIO nodes. systemd-journal lets the
# unprivileged service read its own system-journal entries — required for the
# front-panel Logs page (and the web-UI screen mirror) to tail journalctl.
SUPP_GROUPS_LINE="# (display disabled — no SupplementaryGroups)"
[ "$INSTALL_DISPLAY" = true ] && SUPP_GROUPS_LINE="SupplementaryGroups=spi i2c gpio systemd-journal"
