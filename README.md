...
- **Drone media browser.** A camera/drone shows up as `kind: "camera"`; the daemon walks its `DCIM` folders and exposes `/api/devices/{id}/media` to list, thumbnail, preview, and download its photos and videos. Two transports are covered: **MTP** (older DJI cameras) and **USB Mass Storage** — the latter reaching modern drones such as the DJI Mini 5 Pro, whose footage is read from the SD card via a USB card reader (the Mass Storage volume is mounted read-only). Video previews stream the camera's own low-res `.LRF` proxy clip — the same trick DJI Fly uses for smooth playback — so scrubbing a 4K clip in the browser doesn't choke. Thumbnails come from the device's MTP thumbnail or the photo's embedded EXIF thumbnail, so the gallery loads without downloading full files.
