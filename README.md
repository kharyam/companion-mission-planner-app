...
- **Drone media browser.** A camera/drone shows up as `kind: "camera"`; the daemon walks its `DCIM` folders and exposes `/api/devices/{id}/media` to list, thumbnail, preview, and download its photos and videos. Two transports are covered: **MTP** (older DJI cameras) and **USB Mass Storage** — the latter reaching modern drones such as the DJI Mini 5 Pro, whose footage is read from the SD card via a USB card reader (the Mass Storage volume is mounted read-only). Videos stream for in-browser playback with HTTP range support (seeking, no full download) — a low-res `.LRF` proxy clip when the camera writes one, otherwise the original file straight off the card. Thumbnails come from the device's MTP thumbnail or the photo's embedded EXIF thumbnail, so the gallery loads without downloading full files.
...
- **Embedded admin UI.** A built-in zero-dependency web UI at `/ui` lets you list devices, browse slots, push KMZs, regenerate previews, and download mission KMZs without launching Mission Planner. Selecting a camera/drone instead opens a media gallery with thumbnails, in-browser video preview, and one-click downloads.
