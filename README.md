...
- **Drone media browser.** A camera/drone plugged into USB is also an MTP device. The daemon classifies it (`kind: "camera"`), walks its `DCIM` folders, and exposes `/api/devices/{id}/media` to list, thumbnail, preview, and download its photos and videos. Video previews stream the camera's own low-res `.LRF` proxy clip — the same trick DJI Fly uses for smooth playback — so scrubbing a 4K clip in the browser doesn't choke. Thumbnails come from the device's MTP thumbnail or, failing that, the photo's embedded EXIF thumbnail, so the gallery loads without downloading full files.
